from src.database.api.schemas.user_schemas import PydCreateUser, PydDeleteUser  # TaskHistorySchemaAdd, TaskSchemaAdd, TaskSchemaEdit
...
class UsersService:
    async def add_user(self, uow: IUnitOfWork, user: PydDeleteUser):
    ...
    async def edit_user(self, uow: IUnitOfWork, user_id: int, user: PydDeleteUser):
        users_dict = user.model_dump()
        async with uow:
