...
class Engines:
	async_engine = create_async_engine(url=settings.asyncpg_url, max_overflow=10)  # echo=True,
	async_session_factory = async_sessionmaker(async_engine)

	sync_engine = create_engine(url=settings.psycopg2_url, max_overflow=10)  # echo=True,
